...
void pos_center (int ndim, int n, double pos[],double center[],char ListAB[]) {
...
void report (int what, FILE *fp, int ndim, int n, int steps, double pos[], double vel[], double epot, char AB[]) {
  double ekin;
  double center[ndim*2];
  pos_center(ndim,n,pos,center,AB);
  if (what == 0) {
...
int main (int argc, char * argv[]) {
  ...
  write = fopen("positions.xyz","w");
  file = fopen("nano_A_B_corrige.xyz","r");
  log  = fopen("output_long.log","w");
