...
void forces (int ndim, int n, double rcut[], double box[], double pos[],
		double forc[], double *epot,double halfbox[]) {
...
void evolve (int ndim, int n, double dt, double rcut[], double box[],
		double pos[], double vel[], double forc[], double *epot,double halfbox[]) {
  int i, j, k;
  double mass = 1.0;
  ...
  forces(ndim, n, rcut, box, pos, forc, epot,halfbox);
...
int main (int argc, char * argv[]) {
  ...
  double *box, *halfbox, dt = 0.002, rc = 2.5, epot;
  ...
  forces(ndim,n,rcuts,box,pos,forc,&epot,halfbox);
  report(0,log,ndim,n,0,pos,vel,epot);
  /* Main MD loop */
  for (i=0; i<nsteps; i++) {
    evolve(ndim,n,dt,rcuts,box,pos,vel,forc,&epot,halfbox);
    if (i % 10 == 0) {
