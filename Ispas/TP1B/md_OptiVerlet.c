...
double kinetic (int ndim, int n, double vel[]) {
  ...
  for (j=0; j<n; j++) {
    for (i=0; i<ndim; i++) {
      ekin += vel[j*ndim+i]*vel[j*ndim+i];
    }
  }
  ekin *= 0.5;
  return ekin;
...
void forces (int ndim, int n, double rcut[], double box[], double pos[],
		double forc[], double *epot,double halfbox[], int ListeVerlet[]) {
...
void evolve (int ndim, int n, double dt, double rcut[], double box[],
		double pos[], double vel[], double forc[], double *epot,double halfbox[], int ListeVerlet[]) {
  int i, j, k;
  double mass = 1.0;
  
  for (i=0; i<n; i++)
    for (k=0; k<ndim; k++) {
      pos[i*ndim+k] += vel[i*ndim+k] * dt + 0.5 * forc[i*ndim+k] / mass * dt*dt;
      vel[i*ndim+k] += 0.5 * forc[i*ndim+k] / mass * dt;
    }
  for (i=0; i<n; i++)
    // pbc(ndim, &(pos[i*ndim]), box);
    for (int d=0; d<ndim; d++) {
        if (pos[i*ndim+d] > halfbox[d]) {
      // printf("r sup a demi boite");
          pos[i*ndim+d] -= box[d];
       }
       else if (pos[i*ndim+d] < -halfbox[d]) {
      // printf("r inf a demi boite");
      pos[i*ndim+d] += box[d];
    }
  }

  forces(ndim, n, rcut, box, pos, forc, epot,halfbox,ListeVerlet);
...
int main (int argc, char * argv[]) {
  ...
  double *box, *halfbox, dt = 0.002, rc = 2.5, rverlet = 0.3+rc, epot;
  double *pos, *vel, *forc, *rcuts;
  FILE *file, *log;
  int *Liste;

  rcuts = malloc(4*sizeof(*rcuts));
  rcuts[0] = rc;
  rcuts[1] = rc*rc;
  rcuts[2] = rcuts[1]*rcuts[1]*rcuts[1];
  rcuts[3] = rcuts[2]*rcuts[2];
  ...
  halfbox = malloc(ndim*sizeof(*halfbox));
  Liste = malloc(n*n*sizeof(*Liste));
  ListeVerlet(n,ndim, rverlet, pos,box,Liste);
  for (int i = 0; i < ndim; i++) {
    halfbox[i]=box[i]*0.5;
  }
  forces(ndim,n,rcuts,box,pos,forc,&epot,halfbox,Liste);
  report(0,log,ndim,n,0,pos,vel,epot);
  /* Main MD loop */
  for (i=0; i<nsteps; i++) {
    evolve(ndim,n,dt,rcuts,box,pos,vel,forc,&epot,halfbox,Liste);
    if (i % 10 == 0) {
